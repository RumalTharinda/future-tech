...
$currentYear = date('Y');
$currentMonth = date('m');
$sqlCustomersToday = "SELECT COUNT(*) AS customers30Days FROM tbl_customer WHERE add_date >= DATE_SUB(NOW(), INTERVAL 30 DAY)";
$resultCustomersToday = $db->query($sqlCustomersToday);
$rowCustomersToday = $resultCustomersToday->fetch_assoc();
$customersTodayCount = $rowCustomersToday['customers30Days'];

// Get the count of sales for today
$currentDate = date('Y-m-d');
$sqlSales = "SELECT SUM(biling_total_amount) AS salesCount FROM tbl_biling INNER JOIN tbl_oder ON tbl_oder.order_id=tbl_biling.oder_id WHERE DATE(order_date) > DATE_ADD(NOW(), INTERVAL -30 DAY) AND order_status='paid'";
...
$sqlWaitingOrders = "SELECT COUNT(*) AS waitCount FROM tbl_oder WHERE DATE(order_date) > DATE_ADD(NOW(), INTERVAL -30 DAY) AND order_status='waiting'";
...
$sqlapprovedOrders = "SELECT COUNT(*) AS approvedCount FROM tbl_oder WHERE DATE(order_date) > DATE_ADD(NOW(), INTERVAL -30 DAY) AND order_status='approved'";
...
$sqldeliveredOrders = "SELECT COUNT(*) AS deliveredCount FROM tbl_oder WHERE DATE(order_date) > DATE_ADD(NOW(), INTERVAL -30 DAY) AND order_status='delivered'";
...
// Get the count of orders in delivered status for the day
$sqlpaidOrders = "SELECT COUNT(*) AS paidCount FROM tbl_oder WHERE DATE(order_date) > DATE_ADD(NOW(), INTERVAL -30 DAY) AND order_status='paid'";
