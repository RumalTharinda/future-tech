...
<main id="main" class="main">
    ...
    <section class="section">
        ...
        <div class="card">
            <div class="card-body">
                <h5 class="card-title">Manage Courier</h5>
                ...
                if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                    ...
                    if (!empty($FirstName) && !empty($LastName)) {
                        $db = dbconn();
                        $sql = "SELECT * FROM tbl_couriers WHERE courier_first_name='$FirstName' AND courier_last_name='$LastName'";
                        $result = $db->query($sql);
                    ...
                    if (empty($message)) {
                        $db = dbconn();
                        $sql = "INSERT INTO tbl_couriers(courier_first_name,courier_last_name,courier_title,courier_company_name,courier_company_address_line1,courier_company_address_line2,courier_company_address_line3,courier_mobile,courier_company_phone,courier_company_email)VALUES('$FirstName','$LastName','$Title','$CompanyName','$CompanyAddressLine1','$CompanyAddressLine2','$CompanyAddressLine3','$Mobile','$CompanyPhone','$CompanyEmail')";
                        $db->query($sql);

                        // Display successfully added! message //
                        echo '<div id="liveAlertPlaceholder"><div></div><div><div class="alert alert-success alert-dismissible" role="alert">   <div>New Courier successfully added!</div>   <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div></div><div></div></div>';
                ...
                if ($_SERVER['REQUEST_METHOD'] == 'POST' && @$action == 'delete') {
                    $db = dbconn();
                    $sql = "DELETE FROM tbl_couriers WHERE courier_id='$courier_id' ";
                    $db->query($sql);
                }
                ?>
                                ...
                                <table class="table datatable datatable-table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th data-sortable="true" style="width: 24.919708029197082%;">Courier Name</th>
                                            <th data-sortable="true" style="width: 10.77372262773723%;">Courier's Company Name</th>
    ...
    <div class="card">
        <div class="card-body">
            <h5 class="card-title">New Courier</h5>
            ...
            <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" class="row g-3" novalidate>
                ...
                <div class="col-md-4">
                    <label for="inputState" class="form-label">Courier Title</label>
