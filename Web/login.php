                ...
                <div class="checkout__form">
                    ...
                    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                        ...
                        if (empty($message)) {
                            ...
                            $db = dbconn();
                             $sql = "SELECT * FROM tbl_users LEFT JOIN tbl_customer ON tbl_customer.user_id=tbl_users.user_id WHERE tbl_users.user_name='$UserName' AND tbl_users.user_password='$Password' AND tbl_users.user_status='1'";
                            $result = $db->query($sql);
                            if ($result->num_rows > 0) {
                                $row = $result->fetch_assoc();
                                echo $_SESSION['CUSTOMERUSERID'] = $row['user_id'];
                                $_SESSION['FNAME'] = $row['customer_first_name'];
                                $_SESSION['LNAME'] = $row['customer_last_name'];
                    ...
                    <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" >
                        ...
                        <div class="col-lg-12">
                            <div class="checkout__input">
                                <p>Password<span><i class="fa fa-key fa-fw" aria-hidden="true"></i>*</span></p>
                                <input type="password" class="form-control" id="Password" name="Password" value="<?= @$Password ?>">
                                <div class="text-danger"><?= @$message['err_invalid'] ?></div>  <!-- error message -->
                            </div>
                        </div>

                        <div class="col-lg-12">
                            <p>Don't have account? <a href="register.php" style="color: blue">Create an account</a></p>
                        </div>
